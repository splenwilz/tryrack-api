...
async def get_wardrobe_items(
    category: Optional[str] = Query(
        None,
        description="Filter by category (e.g., 'shirt', 'pants', 'dress')"
    ),
    status_filter: Optional[ItemStatus] = Query(
        None,
        alias="status",
        description="Filter by status (clean, worn, dirty)"
    ),
    ...
    **Authorization:**
    ...
    try:
        ...
        sys.stdout.write(f"[TIMING] Database query (get_wardrobe_items) took {db_query_time:.1f}ms\n")
        sys.stdout.flush()
...
async def delete_wardrobe_item(
    ...
    **Authorization:**
    ...
    wardrobe_service = WardrobeService()
    try:
        deleted = await wardrobe_service.delete_wardrobe_item(db, item_id, current_user.id)
        if not deleted:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Wardrobe item not found"
...
async def mark_item_worn(
    ...
    **Authorization:**
    ...
    try:
        ...
        logger.info(f"Marked wardrobe item {item_id} as worn for user: {current_user.id}")
        return item
    except HTTPException:
        raise
    except Exception as e:
